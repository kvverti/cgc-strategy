...
///
/// A typical GC allocation starts out uninitialized, rooted, and pinned, then becomes unrooted and unpinned once a value
...
pub struct GcHeap<S: ?Sized> {
    strategy: S,
}

impl<S: ?Sized + GcStrategy> GcHeap<S> {
    // todo: figure out how allocation should work
    // do we want to start out rooted and/or pinned? Do we want to use pinning to initialize the value?
    // -> start rooted, but not pinned
    // Do we want to let the strategy determine the layout of the header?
    // note: Send bound here because we eventually want to have dropping handled
    // todo: do we also need a 'static bound, also for dropping?
    pub fn alloc<T: Trace + Send + 'static>(&self, value: T) -> Root<'_, S, T> {
